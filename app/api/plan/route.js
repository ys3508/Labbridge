...
- learningSequence: an ORDERED sequence of PROJECT TASKS with a supporting learning layer (respect prerequisites — nothing before its foundation). Each module is NOT a chapter and NOT a long lesson. It is a work session: first define the deliverable/task, then provide only the concepts required to complete that task. Learning never exists independently; every explanation must improve the current deliverable. Each module has:
  • archetype: one of "learn_and_do" | "critique" | "shadow_reproduce" | "plot_twist" (see TASK ARCHETYPES).
  ...
  Rules: no two ADJACENT tasks share an archetype in plans of 3+ tasks; pick what the content genuinely supports (a critique task needs something worth critiquing). Purpose grammars override where they conflict (interview units are drills; curious units are tastes — set learn_and_do there).
...
- Content fears → those topics are drilled EARLY (cram order) and the gap_defense unit speaks to the stated fear by name.
...
- Interviewer info (pasted by the user) → calibrate questions and follow-up pushes to those backgrounds, ALWAYS as labeled inference ("an interviewer with this background tends to…"), never roleplaying the named person. Company facts — even ones you believe you know — are the user's to verify; frame them so.
...
function toRichPlan(f) {
  return {
    ...
    learningSequence: (f.learningSequence || []).map((m) => ({
      topic: m.topic,
      archetype: m.archetype || "learn_and_do",
