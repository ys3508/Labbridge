...
- interview → QUESTION MAP + DRILL AND REHEARSAL, not simulation. Each unit is ONE likely interview question, in the interviewer's voice. The map's contract: "The questions this round is likely to ask. Not a guarantee — but nothing here is guessed; each one traces to something real." Field semantics shift: topic = the question's short name; context = why interviewers ask it; why = THE RECEIPT — a quoted JD line, resume line, stated round convention, diagnostic result, or pasted interviewer-background line. Every question must trace to one real source; if no source supports a question, drop it. managerRequest = the interviewer question verbatim. task = REHEARSE — write your own answer in your words (the deliverable is an answer-bank entry, e.g. "answer_cohort_design.md"); selfCheck = what a strong answer must contain. Behavioral answers anchor to THEIR actual resume projects, never generic templates. Traps double as PUSHBACK DRILLS: the follow-up an interviewer uses to test if you fold ("are you sure? what if the data is biased?") — rehearsal means surviving two pushes, not just reciting the answer. Capstone = a MOCK INTERVIEW, phases with stage names "Warm-up" | "Core questions" | "Rapid fire" — timed, mixed, drawing on every unit's questions. What accumulates: an answer bank. INTERVIEW FIELD RULES: archetype is retired for interview purpose; return archetype as "" and use section/tag instead. section is exactly one of "fit" | "track_record" | "capability" | "judgment". tag is exactly one of "start_here" | "you_named_this" | "blind_spot" | "". Exactly one question has tag "start_here". Use "you_named_this" when the question comes from the user's challenge/fear/gap words. Use "blind_spot" when the diagnostic was weak and the user did not flag it. THE GAP QUESTION is a tagged question, not a section: tag it "you_named_this" and keep the named-not-dreaded treatment. Questions they ask the interviewers are NOT units; put them in askYourTeam on relevant questions, derived from the JD/company/interviewer background the user supplied.
...
INTERVIEW QUESTION MAP MIX (when PURPOSE is interview):
- Sections are stable; mix changes. fit = why you/us/now. track_record = what they have actually done; behavioral lives here. capability = can they do the job, derived from the JD. judgment = ambiguous calls; preserve judgment if the map trims to three sections.
- The knobs set the mix, not new sections. Round sets breadth/depth: first round = shallow coverage across all four; middle = balanced; final = deeper in the two most relevant sections. Interviewer kind/background sets weights: recruiter → fit; hiring manager → track_record + capability; engineer/peer → capability; exec/VP → judgment + fit; panel → mixed with per-question attribution only when more than one interviewer is supplied. Seniority sets difficulty: first-role gets more fit/capability basics; senior gets less "tell me about yourself" and more judgment.
- Same background + same goal but different interviewer_kind MUST produce visibly different section weights. Do not say a question belongs to a later round; one-round interviews can ask all four sections.
- Default order is worry/prep order. Generate a little above the timeline budget and let skip absorb untagged questions. Tagged questions ("you_named_this", "blind_spot") are not skippable in the UI, so do not make them optional in prose.
...
- learningSequence: an ORDERED sequence of PROJECT TASKS with a supporting learning layer (respect prerequisites — nothing before its foundation). Each module is NOT a chapter and NOT a long lesson. It is a work session: first define the deliverable/task, then provide only the concepts required to complete that task. Learning never exists independently; every explanation must improve the current deliverable. Each module has:
  • archetype: for starting_role/career_move/curious, one of "learn_and_do" | "critique" | "shadow_reproduce" | "plot_twist" (see TASK ARCHETYPES). For interview purpose only, return "" because section replaces archetype.
  • section: for interview purpose, one of "fit" | "track_record" | "capability" | "judgment". For all other purposes, return "".
  • tag: for interview purpose, one of "start_here" | "you_named_this" | "blind_spot" | ""; exactly one "start_here". For all other purposes, return "".
  ...
  • comprehensionCheck (optional but preferred): ONE question after the concept and example — { question, options, answerIndex, explanation }. VARY THE GENRE ACROSS TASKS (a plan of five identical multiple-choice rituals tests nothing; recognition is the lowest form of knowing). Genres:
    · explain_back (free text — PREFER for at least one early task): "Explain to your lead in two sentences why X" — options = [], answerIndex = 0, and explanation = the GRADING KEY: what a strong answer must contain, stated checkably (the app grades their typed answer against it).
    · predict (free text): give a tiny scenario and ask them to predict the outcome BEFORE reasoning is revealed — options = [], explanation = the correct prediction and why.
    · spot_flaw (free text): 2-3 sentences of plausible work containing ONE subtle error from this task's traps — "which sentence is wrong and why?" — options = [], explanation = the flaw and the fix.
  ...
  SCAFFOLDING FADE (the learner GROWS across the plan — the structure must notice): task 1 teaches in full (concept 150-250 words, full example). MIDDLE tasks compress: concept 60-120 words covering ONLY what's new relative to earlier tasks ("you know how claims think now — here's just the new idea"), examples lean. The FINAL task is near-pure production: concept ≤ 60 words or empty "", example may be empty "" — by then they've earned trust, and the plan should visibly hand it over ("you know the drill — this one's yours"). Empty fields make beats disappear; that shrinkage IS the reward. Never write task 5's pages at task 1's altitude.
  TASK ARCHETYPES (set the archetype field; vary shapes across the plan — five identical do-the-assignment tasks bore the learner and misrepresent real work): 
    · learn_and_do — the full shape described in this prompt. Task 1 is usually this.
    · critique — they're HANDED a flawed artifact (givenInputs describe a draft/table/spec "with problems to find"); the deliverable is the review: what's wrong, why, and the fix. Seed the flaws from this task's traps. QC is half of every real job and harder than doing it clean.
    · shadow_reproduce — givenInputs include a COMPLETED example artifact; the task is to reproduce one piece of it independently and compare ("get the same number they got"). The real first-month rite of passage.
    · plot_twist — mid-task the stakeholder CHANGES the ask: one step delivers the twist ("your lead replies: medical affairs needs it tomorrow — cut scope; what do you keep and why?"). Scope-change under pressure is the most job-real skill nobody teaches.
  Rules: no two ADJACENT tasks share an archetype in non-interview plans of 3+ tasks; pick what the content genuinely supports (a critique task needs something worth critiquing). Purpose grammars override where they conflict (interview units use section/tag instead; curious units are tastes — set learn_and_do there).
...
INTERVIEW INTAKE (when the steering notes contain an "INTERVIEW INTAKE" block, honor every line of it):
- Content fears → those topics are drilled EARLY (cram order) and any named gap/fear question speaks to the stated fear by name with tag "you_named_this".
- Personal obstacles → dedicated units where warranted (explaining a layoff/gap is a real unit with real craft), heavier delivery emphasis in criteria, and NEVER test the named fear cold — its first appearance arrives pre-framed and acknowledged.
- Tone signal: "gentle" → NO playful or gamified language anywhere in the plan — warm, precise, zero pep-talk; "playful" permits light energy; default neutral.
- DIAGNOSTIC RESULTS → the plan visibly derives from them: reference what their answers showed ("your story buries the result — that's drill 1") in the relevant units' why/context. If the diagnostic was skipped, say the map was built without hearing them and invite the first rehearsal to calibrate it.
- TRIAGE PRIORITY → order the Question Map and first drills by the listed priority[]; if source=user-override, honor the user's correction over your computed instinct. Runway changes volume, not warmth.
- Round → breadth/depth only. Interviewer kind → the cheap shape signal. Interviewer info (pasted by the user) → stronger section weights and follow-up pushes calibrated to those backgrounds, ALWAYS as labeled inference ("an interviewer with this background tends to…"), never roleplaying the named person. Company facts — even ones you believe you know — are the user's to verify; frame them so.
...
const MODULE = {
  ...
  properties: {
    topic: S, archetype: S, section: S, tag: S, closesGapIndex: I, why: S, context: S, bridgeFromBackground: S,
...
function toRichPlan(f) {
  return {
    ...
    learningSequence: (f.learningSequence || []).map((m) => ({
      topic: m.topic,
      archetype: m.archetype || (m.section ? "" : "learn_and_do"),
      section: m.section || "",
      tag: m.tag || "",
